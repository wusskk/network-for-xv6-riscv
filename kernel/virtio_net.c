...
void
virtio_net_cfg(volatile uint32 *cfg_addr)
{
  struct virtio_net_config *cfg = (struct virtio_net_config *)cfg_addr;

  memmove(local_mac, cfg->mac, ETHADDR_LEN);
...
static void
init_virtq(struct virtq *queue, int qnum)
{
  // initialize queue 0
...
void
virtio_net_send(uint8 *buf, uint32 len, int flag)
{
